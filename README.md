...
> - **叠加层描边** — 可选的一圈对比色轮廓（白 / 黑 / 自定义），同时作用于叠加层形状与动态圆点
...
3. 在"快捷键绑定"页面设置全局快捷键
4. 在"全局选项"页面调整界面、语言、目标显示器和配置文件
5. 最小化窗口后程序会驻留托盘，叠加层持续运行
6. **重点 ！！！**：进入游戏后请使用快捷键操控所需的功能

> ⚠️ **注意事项**
> 1. **全屏独占模式**：部分游戏的全屏独占模式（Exclusive Fullscreen）下叠加层可能无法显示。请将游戏切换为**无边框模式**或**窗口模式**即可正常使用。
> 2. **管理员权限说明**：如果 Steam 或游戏以“管理员身份”运行，MotionStabilizer 也需要以管理员身份运行，否则动态圆点可能不跟随鼠标、全局热键也可能失效。这是 Windows 的权限隔离机制，不是程序故障。
